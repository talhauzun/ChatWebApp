$(document).ready(function () {
    ...
    connection.on("newMessage", function (messageView) {
        ...
        var message = new ChatMessage(messageView.content, messageView.timestamp, messageView.from, isMine, messageView.from.substring(0,2));
    ...
    function AppViewModel() {
        ...
        self.avartarName = ko.computed(function () {
            return self.myName().substring(0, 2);
        });
            ...
            connection.invoke("GetUsers", self.joinedRoom()).then(function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    self.chatUsers.push(new ChatUser(result[i].username,
                        result[i].fullName,
                        result[i].fullName().substring(0, 2),
